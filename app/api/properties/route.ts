...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: apiAccessMessage(error, 'خطا در دریافت اطلاعات ملک‌ها') },
      { status },
    );
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const property = await createProperty({
      ...
      agentId: role === 'AGENT' ? session.user.id : (body.agentId ? String(body.agentId) : undefined),
    });
    ...
    const status = apiAccessStatus(error);
    return NextResponse.json(
      { error: apiAccessMessage(error, 'خطا در ثبت ملک') },
      { status },
    );
  }
}

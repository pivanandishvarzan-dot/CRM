...
async function run(request:Request){if(!authorized(request))return NextResponse.json({error:'UNAUTHORIZED'},{status:401});try{const url=new URL(request.url),limit=Number(url.searchParams.get('limit')||25);const[jobs,smartTasks,riskEscalation,coachingPlans,challenges,recommendations,dailyBrief]=await Promise.all([runDueAutomationJobs(Number.isFinite(limit)?limit:25),generateSmartTasks(),runRiskEscalation(),runCoachingPlanAutomation(),syncChallengeRewards(),evaluateRecommendationOutcomes(),deliverDailyExecutiveBrief()]);const digest=await flushNotificationDigests();const notificationDelivery=await routePendingNotifications();return NextResponse.json({jobs,smartTasks,riskEscalation,coachingPlans,challenges,recommendations,dailyBrief,digest,notificationDelivery})}catch(error){return NextResponse.json({error:error instanceof Error?error.message:'JOB_RUNNER_FAILED'},{status:500})}}

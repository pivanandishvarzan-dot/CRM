...
type OwnerScope={agencyId?:string;agentId?:string};
type OwnerInput = { name:string; phone:string; email?:string; notes?:string; agencyId?:string };
...
export async function listOwners(scope:OwnerScope={}):Promise<OwnerItem[]>{
 if(isDemoMode()) return demoOwnerStore;
 const where=scope.agencyId?{agencyId:scope.agencyId}:scope.agentId?{properties:{some:{agentId:scope.agentId}}}:undefined;
 const rows=await prisma.owner.findMany({where,include:{properties:{where:scope.agentId?{agentId:scope.agentId}:undefined,select:{id:true,agent:{select:{name:true}}}}},orderBy:{createdAt:'desc'}});
...
export async function createOwner(input:OwnerInput):Promise<OwnerItem>{
 ...
 const o=await prisma.owner.create({data:{name:input.name,phone:input.phone,email:input.email||null,notes:input.notes||null,agencyId:input.agencyId||null}});
 return {id:o.id,name:o.name,phone:o.phone,email:o.email??undefined,note:o.notes??'',agent:'—',propertyCount:0};
}
export async function deleteOwner(id:string,scope:OwnerScope={}){
 if(isDemoMode()){demoOwnerStore=demoOwnerStore.filter(x=>x.id!==id);return;}
 if(scope.agencyId||scope.agentId){const owned=await prisma.owner.findFirst({where:{id,...(scope.agencyId?{agencyId:scope.agencyId}:{properties:{some:{agentId:scope.agentId}}})},select:{id:true}});if(!owned)throw new Error('FORBIDDEN_OR_NOT_FOUND');}
